...
Claude Code のローカルスキルは API 経路上に存在しないファイルなので、配布は
git / プラグインの仕事。ただし**「API で Claude を呼ぶアプリ」向けには別経路として
Skills API がある**(§5)— LiteLLM の機能一覧にある「Skills」はそれ。
...
Claude API には **Skills API** がある。スキル(SKILL.md+同梱ファイルの束)を
**組織の Anthropic アカウントにアップロードしてバージョン管理**し、API 呼び出し時に
サーバ側の実行環境へロードして使う仕組み(ベータ機能)。使い手は「API で Claude を
呼ぶ自作アプリ」やサーバ管理型エージェントで、公式のプリビルトスキル
(Excel・PowerPoint・PDF 等)も同じ経路で使える。
LiteLLM の機能一覧にある「Skills」は、この Skills API のゲートウェイ対応のこと。

| | Claude Code のスキル(この教材) | Skills API のスキル |
| --- | --- | --- |
| 置き場所 | 手元のフォルダ(クライアント側) | Anthropic 側(組織アカウント) |
| 配布経路 | git / プラグイン | API アップロード+バージョン管理 |
| 使う人 | Claude Code 利用者 | API で Claude を組み込むアプリ |

**中身は同じ SKILL.md 形式**。この教材で作ったスキル資産は、将来 API アプリに
進んでもそのまま持っていける。
