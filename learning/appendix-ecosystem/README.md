...
LiteLLM の Skills Gateway は「配布所(登録・公開・検索)」を引き受けるもので、
アプリストアと同じモデル。なお、scripts 入りのスキルを「Claude API を組み込んだアプリ」で
使うときだけ、Anthropic に登録する別の仕組み(Skills API)が関係する(§5)。
...
## 5. Skills API — スキルを「API を組み込んだアプリ」で使うとき

**結論から**: この教材のスキル運用(Claude Code+手元のフォルダ)には関係ない話。
「Claude API を組み込んだ自作アプリ」でスキルを使いたくなったときだけ読めばよい。
使い方は2通り。

| スキルの内容 | やり方 |
| --- | --- |
| 手順書(SKILL.md)だけ | アプリが SKILL.md を読んで、プロンプトに含めて送る。**これが普通。特別な仕組みは不要** |
| scripts/ など実行ファイル入り(quarto-typst-pdf, html-craft 型) | **Skills API** を使う(下記) |

**Skills API が必要になる理由**: API 利用時のコード実行は、あなたの PC ではなく
Anthropic 側のサンドボックス(隔離された実行用コンテナ)の中で行われる。だから
scripts を動かすには、ファイル一式をそのコンテナに届ける手段が要る。Skills API は
「スキルのフォルダを zip で事前登録しておくと、API 呼び出しで ID を指定したときに
サンドボックス内へ展開される」という仕組み。登録し直すと版(v1, v2, …)として
記録される。

紛らわしいので注意: LiteLLM のドキュメントで「Skills」は3つの別物を指す。

1. **Skills Gateway** — スキルの社内配布レジストリ。§2 のプラグイン配布の
   マーケットプレイス置き場を LiteLLM が担うもの(保存は LiteLLM 側、実行は
   手元にインストールしたコピー)
2. **litellm-skills** — LiteLLM 自体を管理する(ユーザー追加・キー発行・モデル登録等)
   ためのスキル集。GitHub から取ってきて**自分の PC に置く普通の Agent Skills**で、
   仕組みはレベルAの OSS スキル導入(git で取得してコピー)と同じ
3. **/skills エンドポイント対応** — 上記 Skills API へのリクエストを Anthropic へ
   中継できるという意味
