...
なお、LiteLLM のドキュメントで「Skills」は2つの別物を指すので注意:

1. **litellm-skills** — LiteLLM 自体を管理する(ユーザー追加・キー発行・モデル登録等)
   ためのスキル集。GitHub から取ってきて**自分の PC に置く普通の Agent Skills**で、
   仕組みはレベルAの OSS スキル導入(git で取得してコピー)と同じ
2. **/skills エンドポイント対応** — 上記 Skills API へのリクエストを Anthropic へ
   中継できるという意味

どちらの場合も**LiteLLM 上にスキルが保存されるわけではない**。
