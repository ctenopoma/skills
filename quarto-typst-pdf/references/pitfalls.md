...
- **改訂履歴やメタデータは中間ファイルから取らない。** `build`/`revise` は
  shadow の `.qmd` をレンダするが、これは git 追跡外なので `git log` が空になる。
  元ファイルを別に持ち回る必要がある。
...
- **行の重なりは座標では検出できない。** 表があると行のまとまりが列をまたぐため、
  正常な文書で誤検出する。逆に本当に潰れた文書では検出できなかった。
  当てにならない検出は載せないほうがよい(**偽陽性は検査全体の信用を壊す**)。
- **確実なのは「紙の外に出ているか」。** 版面(余白)は design ごとに違うので、
  紙そのものを基準にする。グリフの枠は字面より少し大きいので 3mm ほど許容する。
- **1ページ目は文字が少なくても咎めない。** 表紙・中扉は正常にまばら。
- **図の中の豆腐は機械では拾えない。** 図はテキスト抽出できないため、
  `check` の豆腐判定にかからない。目で見るしかない。
- **見出しの日本語が1文字ずつ離れる。** テンプレートの `par(justify: true)` が
  見出しにも掛かるため。`#show heading: set par(justify: false)` で直る。
- **`show raw: set par(justify: false)` はやってはいけない。** コードブロックの
  間延びを直そうとすると、逆に1語ずつ改行される。コード側は触らないこと。
