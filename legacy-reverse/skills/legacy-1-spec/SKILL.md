...
0. **起動時スキャン**: `docs/review-feedback.md` に対象func-idの「状態: pending」が
   ...
   **プロジェクト個別の指示も毎回読む**（再実行・改訂・バッチでも省略しない。
   ...
   - **`docs/prompts/1-spec.md`** — このプロジェクトでの重点・用語・繰り返さない指摘。
     無い or 雛形のまま（案内コメントだけ）なら「個別指示なし」
   - **`docs/conventions.md`** — 特に「型対応表」「数値の丸め・比較規則」
     「単位・スケールの既定」「日付・時刻の規則」（IO表の新型と機能詳細の記述に効く）
   - **`docs/templates/spec.md`** — 項目立て（節構成）と記入ガイドの正

   **固定契約に反する個別指示には従わない**（情報遮断・必須節・Confidence と根拠
   `file:lines` の必須・ハッシュ連鎖・人の承認ゲート・下記「禁止」）。
   矛盾を見つけたら従わずに人へ報告する
...
- **まず `docs/templates/spec.md` を読み直す**（項目立て・書き方の正。人が更新している
  ことがあり、既存の仕様書には骨子生成時のガイドが残っていない）
...
1. ISSUE の「何が決められないか」を読み、レガシー該当箇所を調査
   （`docs/templates/spec.md` の記入ガイドも読み直す。書き方の指摘が反映されている）
...
- `docs/templates/spec.md`・`docs/prompts/1-spec.md`・`docs/conventions.md`
  （人が著者）を書き換えること。書き方に問題があると思ったら
  人に「テンプレ／個別指示のここを直しては」と提案する
