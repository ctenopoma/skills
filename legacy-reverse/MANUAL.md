...
> **最初に1回だけ: ツールの実行許可を確認してください。** ブラウザからの実行は
> headless Claude で動くため、許可ダイアログに答えられません。許可が無いと
> **応答は返るのにファイルが書かれず、全件が同じ理由でNG**になります。方法は2つ:
> (1) `<project>/.claude/settings.json` の `permissions.allow` に
> `Read` / `Edit` / `Write` / `Bash(python *)`（MCP登録済みなら `mcp__legacy-reverse__*` も）を
> 並べる。(2) 信頼できる閉じた環境なら、サーバを
> `serve_site.py --root . --skip-permissions` で起動する（ブラウザからの全実行に
> `--dangerously-skip-permissions` が付きます。共有マシンでは使わないこと）。
...
| ブラウザ実行で、応答はあるのにファイルが更新されない（全件が同じ理由でNG） | headless がツールの実行許可を得られていない | 応答ログに「許可」「permission」の文言が出る。settings.json の permissions.allow を設定するか、閉じた環境なら `serve_site.py --skip-permissions` で起動し直す |
