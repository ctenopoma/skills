...
| フェーズ | コマンド | 人がやること |
|---------|---------|-------------|
| ⓪ 解析 | `/legacy-0-analyze` | レガシーの場所・言語・新パッケージ名を答える。型対応表・規約を一緒に確定（Fortran と C/C++ の関数列挙は静的解析プログラムが行い、AIは説明の充填と抽出レポートの確認のみ。Fortran↔C の呼び出しも自動でリンクされる）。0割などの扱いを決める（後述） |
...
- **⭐優先**: 押した関数が次の走査で**先頭に割り込みます**（実行中の1件が
  ...
  ターミナル派は `pipeline.py priority F-xxxx` で同じことができます（後述）
...
**⭐優先はターミナルからも操作できます**（ブラウザの⭐ボタンと同じ効果。
...
ブラウザの「変数辞書」ページ（ナビバーに自動で出ます）が承認画面です。
並び順は**「多くの関数で使われているもの」から、「確認が要るもの」を先に**——
つまり上から順に片付ければ、効果の大きいものから終わります。

- **rank A/B はチェックボックスで一括承認**。根拠が明示的なので、目を通して
  まとめて「承認」で構いません
- **rank C/D は1件ずつ**。意味の欄を直してから「修正して承認」を押します
  （直した内容がそのまま確定します）
- 1回の承認で、その変数が出てくる**全関数に効きます**

チャットでも同じことができます（「V-0001 と V-0002 を承認して」「V-0003 は
年間税率に直して承認して」）。どちらでも記録される内容は同じです。

承認するとその場で仕様書の入出力表への転記まで走るので、**AI がバッチで解釈を
回している最中でも、あなたは並行して承認を進められます**。
...
- ナビバー: ドメイン知識 / 規約 / **変数辞書**（作られていれば自動で出ます。
  未承認があるときは承認パネル付き）/ ⑥完了検証 / ⑦分析 / 新コード詳細(API) /
...
| `docs/exception-policy.md`（例外ポリシー登録簿） | ⭕ | あなたが承認する規約。AIに「登録して」と言うのが基本（列の並びは変えないこと） |
| `docs/specs/`（仕様書） | △ | 編集可。ハッシュ連鎖が下流を stale にして再確認が走る（設計どおり） |
| WBS・テスト結果・完了検証レポート | ❌ | 自動生成。次の再生成で消える |
| 変数辞書・未決定キュー・矛盾候補のページ | ❌ | 自動生成。意味を直すときは画面の「修正して承認」か、AIに頼む |
...
docs/
  ...
  variables.qmd        変数辞書＋承認パネル（自動生成・手編集禁止）
