...
`docs/specs/<fid>.md` が①未着手（skeleton）の間、そのページに **「①を実行する」** ボタンが
出ます。押すと `/legacy-1-spec F-xxxx` とチャットで打つのと同じ処理（headless Claude を
1回起動して draft まで作る）がバックグラウンドで走り、進捗はページ上と `/pipeline.html`
...
- 現在は①（仕様書）のみ対応の試作です。②以降への拡張は今後の課題
