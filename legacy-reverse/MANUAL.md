...
- **⑥完了検証はWBSのトップページから直接実行できます**。「⑥を実行する」ボタンは
  常に表示されており、途中経過の確認にも使えます（LLMを呼ばないので数十秒で終わります）
...
- **連続実行（まとめて回す）**: `/pipeline.html` の「連続実行」から開始すると、
  ...
  自動で1件ずつ実行し続けます。承認待ち・裁定待ちはスキップするので、
  あなたは実行中もWBS上で承認・裁定だけしていけば、次の走査で自動的に拾われます。
  上限件数・予算（$）も指定でき、「停止」でいつでも止められます
- 現在は①〜⑥対応の試作です。⑦（探索的な改善ループ）は形が大きく異なるため
  別途設計します。⓪（骨子生成）と serve_site.py の起動はこれまで通り
  チャット/ターミナルです（ブラウザ自体がこのサーバの上で動くため）
