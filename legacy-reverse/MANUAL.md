...
- **数百件をまとめて流すときは2通り**あります。
  **①だけを全件先に流す**なら CLI の無人バッチ（`pipeline.py`。①専用です）、
  **関数を①〜⑤まで通しで進める**ならブラウザの「連続実行」（後述のバッチ実行状況ページ）
...
**数百〜2000件の①全件実行は無人ドライバ**で行います（**①専用**です。
②以降も続けて進めたいときはブラウザの「連続実行」を使ってください）。
...
python <LR>/scripts/pipeline.py spec --root .                  # 全件（中断後も同じコマンドで再開）
```
