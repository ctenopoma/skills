...
## ①②の作成もブラウザから起動できます（試作機能）

①仕様書ページに、状況に応じて **「①を実行する」** または **「②を実行する」** ボタンが
出ます（①未着手のときは①、①reviewed済みでまだ②が無いときは②）。押すとチャットで
`/legacy-1-spec F-xxxx` / `/legacy-2-testspec F-xxxx` と打つのと同じ処理（headless Claude
を1回起動して成果物を作る）がバックグラウンドで走り、進捗はページ上と `/pipeline.html`
の両方で見えます。終わると自動でページが更新され、承認ウィジェットに切り替わります。
...
- 無人バッチ（pipeline.py）が実行中のときは、ブラウザからの実行は開始できません
  （同じ実行枠を取り合う設計。バッチの完了を待ってから使う）
- 現在は①②のみ対応の試作です。③④への拡張は今後の課題（hookガード連携が追加で要る）
...
概要・Confidence 内訳・質問を並べて持ってきます。あなたは表を見て
「全部OK」または「F-xxxx は修正: 〜」と返すだけです。
